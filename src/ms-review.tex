...
\abstract{
	...
	this article we present the first major of \gammapy, the Long-term Support (LTS) v1.0, a
...
The lists of \gammary events are then used to derive scientific results, such as spectra, sky maps
...
light curves and catalogs. The methods and tools applied here are more generic and are broadly
...
in the \code{gammapy.data} (see \ref{ssec:gammapy-data}) and the \code{gammapy.irf}
...
implemented in the \code{gammapy.makers} subpackage (see \ref{ssec:gammapy-makers}).
...
a multi-dimensional map of the PSF in sky coordinates. At each position, a PSF
...
\ref{sec:gammaray-data-analysis} (and in detail in \ref{ssec:gammapy-makers}) are a set of binned counts, background and IRF maps,
...
the class name.
The spectral models include a special class of normed models, named using the \code{NormSpectralModel} suffix.
These spectral models feature a dimension-less norm parameter
...
model is fitted (or 'scaled') in the  restricted data range only.
...
\begin{figure*}[t]
	...
	\caption{Example of a 3D analysis for simulated sources using the \cta \irfs. The
...
source in each time (and optionally energy) band per observation, keeping other parameters constant.
...
ON/OFF counts, and a $\chi^2$ accounting for the flux points. For \fermi, a 
...
few thousands of observations. The object detection step consists a minima in
...
the "scikit-learn" package~\citep{scikit-learn}. Tests on simulations shown that it
...
to the set of \gammaray catalogs available in \code{gammapy.catalog}. However
...
institutes in the very high-energy astrophysics
