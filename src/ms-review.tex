...
\abstract{
	...
	{Context} In the last two decades the measurement of \gammaray emission from the universe has
	evolved from a niche area of closed collaborations operating with proprietary data and software into
	an active and exciting field of astrophysics. The next generation of \gammaray instruments,
	such as the Cherenkov Telescope Array (CTA), will be operated as open observatories with publicly available
	data, which will require open high-level astronomy software customised for high energy astrophysics.

	{Aims} In this article, we present the first the Long-term Support (LTS) release of \gammapy v1.0. \gammapy
	is a community-developed, open-source Python package for \gammaray astronomy. Integrated into the general
	python eco-system, it is built on Numpy, Scipy and Astropy and uses open FITS based data formats for serialisation.
	It provides a uniform platform for reduction and modelling of data from many \gammaray instruments.

	{Methods} Starting from event list and instrument response functions at the so called "Data Level 3",
	\gammapy provides pipelines for reduction of the input data to binned WCS, HEALPix or region based data structures.
	A variety of background reduction methods, including traditional techniques like the Reflected and Ring regions,
	as well as novel 3D Field of View Likelihood techniques, are supported. The binned data can then be modelled
	using Poisson maximum likelihood fitting. While a variety of in-built spectral, temporal and spatial models
	are supplied, it also supports implementation of custom user defined models. Estimation of flux points, likelihood
	profiles and light curves are also implemented.

	{Results} The flexible API allows a joint likelihood analysis between different datasets,
	thus providing a simple platform for handling time dependent instrument responses,
	different events classes, or combining data from multiple separate instruments. The results are continuously
	validated with previously published ones. We show analysis examples using open data from various observatories: \fermi., \hess , MAGIC, CTA (simulated),
	and HAWC. 
}
